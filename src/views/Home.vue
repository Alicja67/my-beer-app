<template>
  <div class="grid">
    ...
    <div class="col-12 surface-900 p-4 top-0 flex flex-row justify-content-between align-content-center">
    ...
    <beers-list class="col-12" :beers="results" :searchName="searchName"></beers-list>
    <Paginator
...
export default defineComponent({
  ...
  setup() {
    ...
    const onPage = (event: any) => {
      url.value = `https://api.punkapi.com/v2/beers?page=${event.page + 1}&per_page=25`;
      getApiData();
    };
    ...
    const handleSearch = () => {
      if (searchName.value !== '' && searchIbu.value === null) {
        ...
        if (!validateIbu(searchIbu.value)) {
          showInfo('warn', 'Zły IBU', 'proszę o podanie liczby');
          handleReset();
        }
        if (ibu_gt.value) {
        ...
        searchName.value = '';
        // searchIbu.value = null;
        showInfo('warn', 'Brak danych', 'Wpisz nazwę/IBU w celu filtrowania danych');
      }
    };
